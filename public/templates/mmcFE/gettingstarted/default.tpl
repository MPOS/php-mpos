{include file="global/block_header.tpl" BLOCK_HEADER="Getting Started Guide" BLOCK_STYLE="clear:none;"}
    ...
    <li>3. <strong>Configure your miner.</strong>
      ...
      <table width="50%">
        <tbody>
          ...
          <tr><td>PORT:</td><td><kbd>{$SITESTRATUMPORT}</kbd></td></tr>
      ...
      <pre>./cgminer --scrypt -o stratum+tcp://{$smarty.server.SERVER_NAME}:{$SITESTRATUMPORT} -u <em>Weblogin</em>.<em>Worker</em> -p <em>Worker password</em></pre>
    ...
    <li>4. <strong>Create a {$SITECOINNAME} address to recieve payments.</strong>
      <ul>
        <li> Downloading the client &amp; block chain: 	Download the {$SITECOINNAME} client from the <a href="{$SITECOINURL}" target="_blank">here</a>.
