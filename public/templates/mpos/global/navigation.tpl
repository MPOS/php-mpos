    ...
    {if !$GLOBAL.config.disable_inbox}
        <li class="icon-inbox"><a href="{$smarty.server.PHP_SELF}?page=account&action=inbox">Inbox {if $GLOBAL.userdata.inbox_unread}({$GLOBAL.userdata.inbox_unread}){/if}</a></li>
    {/if}
    </ul>
    </li>
    {/if}
