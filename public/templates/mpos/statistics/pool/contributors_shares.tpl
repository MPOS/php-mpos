...
{section shares $CONTRIBSHARES}
      <tr{if $GLOBAL.userdata.username|default:""|lower == $CONTRIBSHARES[shares].account|lower}{assign var=listed value=1} style="background-color:#99EB99;"{else} class="{cycle values="odd,even"}"{/if}>
        ...
        <td align="center">{if $CONTRIBSHARES[shares].donate_percent > 0 AND $CONTRIBSHARES[shares].donate_percent < 2}<i class="icon-star-empty"></i>{elseif $CONTRIBHASHES[shares].donate_percent >= 2}<i class="icon-award"></i>{else}<i class="icon-thumbs-down"></i>{/if}</td>
...
{if $listed != 1 && $GLOBAL.userdata.username|default:"" && $GLOBAL.userdata.shares.valid|default:"0" > 0}
      <tr>
        ...
        <td align="center">{if $GLOBAL.userdata.donate_percent > 0 AND $GLOBAL.userdata.donate_percent < 2}<i class="icon-star-empty"></i>{elseif $GLOBAL.userdata.donate_percent >= 2}<i class="icon-award"></i>{else}<i class="icon-thumbs-down"></i>{/if}</td>
