...
{section contrib $CONTRIBHASHES}
      ...
      <tr{if $GLOBAL.userdata.username|default:""|lower == $CONTRIBHASHES[contrib].account|lower}{assign var=listed value=1} style="background-color:#99EB99;"{else} class="{cycle values="odd,even"}"{/if}>
        ...
        <td align="center">{if $CONTRIBHASHES[contrib].donate_percent|default:"0" > 0 AND $CONTRIBHASHES[contrib].donate_percent|default:"0" < 2}<i class="icon-star-empty"></i>{elseif $CONTRIBHASHES[contrib].donate_percent|default:"0" >= 2}<i class="icon-award"></i>{else}<i class="icon-block"></i>{/if}</td>
...
{if $listed != 1 && $GLOBAL.userdata.username|default:"" && $GLOBAL.userdata.rawhashrate|default:"0" > 0}
      ...
      <tr>
        ...
        <td align="center">{if $GLOBAL.userdata.donate_percent|default:"0" > 0 AND $GLOBAL.userdata.donate_percent|default:"0" < 2}<i class="icon-star-empty"></i>{elseif $GLOBAL.userdata.donate_percent|default:"0" >= 2}<i class="icon-award"></i>{else}<i class="icon-block"></i>{/if}</td>
