...
{section name=setting loop=$SETTINGS.$TAB}
        <fieldset>
          ...
          {else if $SETTINGS.$TAB[setting].type == 'text'}
            <input type="text" height="{$SETTINGS.$TAB[setting].height|default:"1"}" size="{$SETTINGS.$TAB[setting].size}" name="data[{$SETTINGS.$TAB[setting].name}]" value="{$SETTINGS.$TAB[setting].value|default:$SETTINGS.$TAB[setting].default}" />
