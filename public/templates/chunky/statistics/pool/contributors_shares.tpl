...
{section shares $CONTRIBSHARES}
      <tr{if $GLOBAL.userdata.username|default:""|lower == $CONTRIBSHARES[shares].account|lower}{assign var=listed value=1} style="background-color:#99EB99;"{else} class="{cycle values="odd,even"}"{/if}>
        ...
        <td>{if $CONTRIBSHARES[shares].is_anonymous|default:"0" == 1 && $GLOBAL.userdata.is_admin|default:"0" == 0}anonymous{else}{substr($CONTRIBSHARES[shares].account, 0, strlen($CONTRIBSHARES[shares].account) - rand(1, 3))|escape}{substr(md5($CONTRIBSHARES[shares].account), 0, 5)}***{/if}</td>
