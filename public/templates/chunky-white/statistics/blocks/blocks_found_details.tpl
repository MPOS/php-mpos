...
{math assign="percentage" equation="shares / estshares * 100" shares=$BLOCKSFOUND[block].shares|default:"0" estshares=$BLOCKSFOUND[block].estshares}
          <span class="{if ($percentage <= 100)}green{else}red{/if}">{$percentage|number_format:"2"}</span>
        </td>
      </tr>
{/section}
    <tr>
      ...
      <td align="right" style="padding-right: 25px;">{if $count > 0}<span class="{if (($totalshares / $totalexpectedshares * 100) <= 100)}green{else}red{/if}">{($totalshares / $totalexpectedshares * 100)|number_format:"2"}</span>{else}0{/if}</td>
    </tr>
    </tbody>
  </table>
  <footer>
    {if $GLOBAL.config.payout_system != 'pps'}<ul><li>Note: Round Earnings are not credited until <span class="orange">{$GLOBAL.confirmations}</span> confirms.</li></ul>{/if}
  </footer>
</article>
