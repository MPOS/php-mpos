...
$(document).ready(function(){
  ...
  var jqPlotOverviewOptions = {
    ...
    axes: {
      yaxis:  { min: 0, pad: 1.25, label: 'Hashrate',  labelOptions:{ fontFamily:'Helvetica', fontSize: '14pt', textColor: '#ffffff', fillColor: '#ffffff'},  labelRenderer: $.jqplot.CanvasAxisLabelRenderer },
              y3axis: { min: 0, pad: 1.25, label: 'Sharerate', labelOptions:{ fontFamily:'Helvetica', fontSize: '14pt', textColor: '#ffffff', fillColor: '#ffffff'}, labelRenderer: $.jqplot.CanvasAxisLabelRenderer },
  ...
  var jqPlotShareinfoOptions = {
    ...
    axes: {
yaxis: { tickOptions: { angle: -90 }, ticks:  [ 'valid', 'invalid' ], tickOptions: { textColor: '#fff', fillColor: '#fff' }, renderer: $.jqplot.CategoryAxisRenderer },
      xaxis: { tickOptions: { angle: -17, textColor: '#fff', fillColor: '#fff' }, pointLabels: { show: true } }
    }
  };
  ...
  g1 = new JustGage({id: "nethashrate", value: parseFloat({/literal}{$GLOBAL.nethashrate}{literal}).toFixed(2), min: 0, max: Math.round({/literal}{$GLOBAL.nethashrate}{literal} * 2), title: "Net Hashrate", gaugeColor: '#6f7a8a', valueFontColor: '#fff', shadowOpacity : 0.8, shadowSize : 0, shadowVerticalOffset : 10, label: "{/literal}{$GLOBAL.hashunits.network}{literal}"});
  g2 = new JustGage({id: "poolhashrate", value: parseFloat({/literal}{$GLOBAL.hashrate}{literal}).toFixed(2), min: 0, max: Math.round({/literal}{$GLOBAL.hashrate}{literal}* 2), title: "Pool Hashrate", gaugeColor: '#6f7a8a', valueFontColor: '#fff', shadowOpacity : 0.8, shadowSize : 0, shadowVerticalOffset : 10, label: "{/literal}{$GLOBAL.hashunits.pool}{literal}"});
  g3 = new JustGage({id: "hashrate", value: parseFloat({/literal}{$GLOBAL.userdata.hashrate}{literal}).toFixed(2), min: 0, max: Math.round({/literal}{$GLOBAL.userdata.hashrate}{literal} * 2), title: "Hashrate", gaugeColor: '#6f7a8a', valueFontColor: '#fff', shadowOpacity : 0.8, shadowSize : 0, shadowVerticalOffset : 10, label: "{/literal}{$GLOBAL.hashunits.personal}{literal}"});
  if ({/literal}{$GLOBAL.userdata.sharerate}{literal} > 1) {
    initSharerate = {/literal}{$GLOBAL.userdata.sharerate}{literal} * 2
  } else {
    initSharerate = 1
  }
  g4 = new JustGage({id: "sharerate", value: parseFloat({/literal}{$GLOBAL.userdata.sharerate}{literal}).toFixed(2), min: 0, max: Math.round(initSharerate), gaugeColor: '#6f7a8a', valueFontColor: '#fff', shadowOpacity : 0.8, shadowSize : 0, shadowVerticalOffset : 10, title: "Sharerate", label: "shares/s"});
  g5 = new JustGage({id: "querytime", value: parseFloat(0).toFixed(0), min: 0, max: Math.round(5 * 100), gaugeColor: '#6f7a8a', valueFontColor: '#fff', shadowOpacity : 0.8, shadowSize : 0, shadowVerticalOffset : 10, title: "Querytime", label: "ms"});
