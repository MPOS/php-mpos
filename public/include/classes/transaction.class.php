...
class Transaction extends Base {
  ...
  public function setConvertibleArchived($account_id, $txid) {
    ...
    $stmt = $this->mysqli->prepare("
      ...
      WHERE t.archived = 0
      AND (
           ( t.type in ('Convertible', 'Fee', 'Donation') AND t.account_id = ? AND b.confirmations >= ? ) OR
           ( t.type = 'Convertible_Transfer' AND t.account_id = ? )
      )");
     if ($this->checkStmt($stmt) && $stmt->bind_param('iii', $account_id, $this->config['confirmations'], $account_id) && $stmt->execute())
      return true;
    return $this->sqlError();
  }
 ...
 public function getConvertibleQueue($limit=250) {
    ...
    $stmt = $this->mysqli->prepare("
        ...
        IFNULL(
          ROUND(
            (
              ...
              SUM( IF( ( t.type in ('Donation', 'Fee') AND b.confirmations >= " . $this->config['confirmations'] . "), t.amount, 0 ) )
            ), 8
          ), 0
        ) AS amount,
      t.convertible
      ...
      LEFT JOIN " . $this->user->getTableName() . " AS a
      ON t.account_id = a.id
      WHERE t.type in ('Convertible', 'Convertible_Transfer', 'Donation', 'Fee') AND t.archived = 0
