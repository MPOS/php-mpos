...
if (isset($_REQUEST['filter'])) {
  ...
  if ($aUsers = $statistics->getAllUserStats($_REQUEST['filter'], $iLimit, $start)) {
    ...
    foreach ($aUsers as $iKey => $aUser) {
      ...
      $aUser['hashrate'] = $statistics->getUserHashrate($aUser['username']);

      if ($config['payout_system'] == 'pps') {
        $aUser['sharerate'] = $statistics->getUserSharerate($aUser['username']);
        $aUser['difficulty'] = $statistics->getUserShareDifficulty($aUser['username']);
