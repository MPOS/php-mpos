...
class Api extends Base
{
    ...
    public function isActive($error = true)
    {
   ...
   **/
  public function get_json($data, $force = false)
  {
      $json = json_encode(
   ...
   **/
  public function checkAccess($user_id, $get_id = null)
  {
      if (!empty($get_id) && is_array($get_id)) {
          die('Access denied');
      }
      if (is_array($user_id)) {
          die('Access denied');
      ...
      header('HTTP/1.1 401 Unauthorized');
          die('Access denied');
      } elseif ($this->user->isAdmin($user_id) && !empty($get_id)) {
