...
{
    /**
     * @var int enable (1) or disable (0) debugging
     */
    private $DEBUG;

    /**
     * @var array Data array with debugging information
     */
    private $arrDebugInfo;

    /**
     * @var float Start time of our debugger
     */
     ...
     * Construct our class.
     *
     * @param int $DEBUG [optional] Enable (>=1) or disable (0) debugging
     *
     * @return none
     */
    public function __construct($log, $DEBUG = 0)
    {
        $this->log = $log;
        $this->DEBUG = $DEBUG;
        if ($DEBUG >= 1) {
     ...
     *
     * @param int $DEBUG our debugging level
     */
    ...
    {
        $bth = debug_backtrace();
        while (strpos($bth[0]['file'], 'classes/debug') != false) {
            array_shift($bth);
        }
        foreach ($bth as $x) {
            $backtrace[] = array(
     ...
     * @return none
     */
    public function append($msg, $debug = 1)
    {
        if ($this->DEBUG >= $debug) {
